...
try:
    # Carrega a música (substitua pelo caminho do seu arquivo de música)
    pygame.mixer.music.load("assets/time_is_up.mp3")  # Substitua pelo seu arquivo de música
    pygame.mixer.music.set_volume(0.5)  # Volume entre 0 e 1
except:
    print("Erro ao carregar a música do jogo")
...
while rodando:
    ...
    if estado_jogo == JOGANDO:
        ...
        if tempo_restante <= 0:
            ...
            pygame.mixer.music.stop()  # Para a música quando o jogo acaba
        ...
        for moeda in moedas[:]:
            if personagem.rect.colliderect(moeda):
                moedas.remove(moeda)
                pontuacao += 10
                # Efeito sonoro ao pegar moeda (opcional)
                try:
                    efeito_coin = pygame.mixer.Sound("assets/coin.wav")  # Substitua pelo seu arquivo
                    efeito_coin.play()
                except:
                    pass
